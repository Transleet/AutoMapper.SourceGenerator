...
";
    ...
    {
        context.RegisterPostInitializationOutput(i => i.AddSource("AdaptAttribute.g.cs", AttributeText));

        IncrementalValuesProvider<object> attributeDeclarations = context
            .SyntaxProvider
        ...
        IncrementalValueProvider<(Compilation, ImmutableArray<object>)> compilationAndClasses
    ...
    private static void Execute(Compilation compilation, ImmutableArray<object> promisedGeneratingTypeAndPropertyInfos, SourceProductionContext context)
    {
        var emitter = new Emitter();
        var sources = emitter.Emit(promisedGeneratingTypeAndPropertyInfos);
